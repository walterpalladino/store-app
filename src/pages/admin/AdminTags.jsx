...
import {
  RefreshOutlined, CloseOutlined, SellOutlined,
...
// ── Tag form drawer (Add + Edit) ─────────────────────────────────────────────
function TagFormDrawer({ tag, isNew, onClose, onSaved, merchantFetch }) {
  const isOpen = isNew || !!tag
  const [form,     setForm]     = useState(() => isNew ? { ...EMPTY_FORM } : { name: tag?.name ?? '' })
  ...
  useEffect(() => {
    ...
    else if (tag) setForm({ name: tag.name ?? '' })
    setErrors({}); setApiError('')
    // Intentionally keyed on the tag id — reset only when a *different* tag is
    // opened, not on every re-render that yields a new object.
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body   = { name }
      const url    = isNew ? API.tags.list : API.tags.byId(tag.id)
      const method = isNew ? 'POST' : 'PATCH'
      const res    = await merchantFetch(url, { method, body: JSON.stringify(body) })
      const saved  = await unwrap(res)   // { id, name } (throws on success:false)
      onSaved(
        { id: saved.id ?? tag?.id, name: saved.name ?? name },
        isNew,
      )
    } catch (err) {
    ...
    <Drawer anchor="right" open={isOpen} onClose={onClose}
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 3, py: 2, borderBottom: '1px solid', borderColor: 'divider', flexShrink: 0 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {isNew
            ...
            : <Chip label={`ID: ${tag?.id}`} size="small" icon={<LockOutlined sx={{ fontSize: '12px !important', color: 'text.secondary !important' }} />} sx={{ height: 18, fontSize: '0.58rem', bgcolor: 'rgba(26,26,26,0.06)', letterSpacing: '0.06em' }} />}
        </Box>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', p: 3 }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <TextField
              fullWidth size="small" name="name" label="Tag Name *"
              value={form.name} onChange={handleNameChange}
              error={!!errors.name}
              helperText={errors.name ?? 'A unique label used to relate products (e.g. waterproof).'}
...
export default function AdminTags() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await unwrap(res)   // [{ id, name }, …]
  ...
  const handleSaved = (saved, isNew) => {
    if (isNew) {
      ...
      showToast(`Tag "${saved.name}" created successfully.`)
      setAddOpen(false)
    } else {
      setTags((prev) => prev.map((t) => t.id === saved.id ? { ...t, ...saved } : t))
      showToast(`Tag "${saved.name}" updated successfully.`)
      setSelected(null)
    }
  }
    ...
    <Fade in>
      <Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
          <Box>
            <Typography sx={{ fontFamily: '"Cormorant Garamond", serif', fontSize: '1.35rem', fontWeight: 400, lineHeight: 1.2, mb: 0.4 }}>Tags</Typography>
            <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.78rem' }}>Create and edit the tags used to label your products.</Typography>
          </Box>
          <Button
            variant="contained"
        ...
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 3 }}>
          <StatCard icon={<SellOutlined sx={{ fontSize: 16 }} />} label="Total Tags" value={loading ? '…' : tags.length} />
        </Box>

        {/* ── Toolbar ── */}
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 1 }}>
          <Typography sx={{ fontSize: '0.72rem', color: 'text.secondary' }}>
            {loading ? 'Loading…' : `${tags.length} ${tags.length === 1 ? 'tag' : 'tags'}`}
          </Typography>
                      ...
                      <TableRow key={i}>
                        {Array.from({ length: 3 }).map((_, j) => (
                          <TableCell key={j} sx={{ py: 1.5, px: 2 }}>
                            <Box sx={{ height: 16, borderRadius: 1, bgcolor: 'rgba(26,26,26,0.06)', width: j === 0 ? 160 : 60 }} />
                          </TableCell>
                        ))}
                      </TableRow>
                    ))
                    ...
                    ? (
                        <TableRow>
                          <TableCell colSpan={3} sx={{ py: 5, textAlign: 'center' }}>
                            <Typography sx={{ fontSize: '0.85rem', color: 'text.secondary' }}>No tags yet. Create your first one.</Typography>
                          </TableCell>
                        </TableRow>
                      )
                    : tags.map((t, idx) => (
                        <TableRow key={t.id} onClick={() => { setSelected(t); setAddOpen(false) }}
                          sx={{
                            cursor: 'pointer',
                          ...
                          <TableCell sx={{ py: 1.5, px: 2 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                              <Box sx={{ width: 32, height: 32, borderRadius: 1, bgcolor: 'rgba(200,169,110,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                <SellOutlined sx={{ fontSize: 16, color: 'secondary.dark' }} />
                              </Box>
                              <Typography sx={{ fontSize: '0.9rem', fontFamily: '"Cormorant Garamond", serif', fontWeight: 400 }}>{t.name}</Typography>
                            </Box>
                          </TableCell>
                          <TableCell sx={{ py: 1.5, px: 2 }}>
                            <Chip label={t.id} size="small" sx={{ height: 18, fontSize: '0.62rem', fontFamily: 'monospace', bgcolor: 'rgba(26,26,26,0.05)' }} />
                          </TableCell>
                          <TableCell align="right" sx={{ py: 1.5, px: 2 }}>
                            <Tooltip title="Edit tag" arrow>
                              <IconButton size="small" onClick={(e) => { e.stopPropagation(); setSelected(t); setAddOpen(false) }}
